...
def recreate_file_struct_locally(dataset_path, image_dir_name, metadata_dir_name, repo_id):
    ...
    return (
        all_image_subdirs,
        all_metadata_subdirs,
    )  # list of all subdir paths and csvs for each data source
...
def batch_convert_png_jpg(png_paths):
    ...
    with open(temp_dir + "/ffmpeg_commands.sh", "a+") as f:
        for i in range(len(png_paths)):
            f.writelines(
                f"ffmpeg -loglevel quiet -i {png_paths[i]} -vf scale=256:256 {jpg_paths[i]}\n"
            )
...
def download_image_files_as_jpg(
    all_subdirs, repo_id, local_dataset_dir, train_reduce_factor=0.075, val_reduce_factor=0.5
):
    jpg_paths_by_source = {}
    ...
    for i in tqdm(range(len(all_subdirs))):
        for subdir in all_subdirs[i]:
            ...
            files = list(
                list_repo_tree(
                    repo_id=repo_id, repo_type="dataset", path_in_repo=subdir, recursive=True
                )
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--repo_id",
        type=str,
        default="InfImagine/FakeImageDataset",
        help="dataset repo id listed on huggingface",
    )
    args = parser.parse_args()

    # construct local folder system matching data repo on HuggingFace servers
    all_image_subdirs, all_metadata_subdirs = recreate_file_struct_locally(
        args.local_dataset_path, args.image_dir, args.metadata_dir, args.repo_id
    )
    # print([all_image_subdirs[0]])
    # download all images in jpg format
    jpg_paths_by_source = download_image_files_as_jpg(
        all_image_subdirs, args.repo_id, args.local_dataset_path
    )

...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpu_id", type=int, default=0, help="ID value (i.e., 0 or 1) of GPU to be used"
    )
    ...
    if (opt.real_path is None) or (opt.fake_path is None) or (opt.data_mode is None):
        ...
        dataset_paths = [
            dict(real_path=opt.real_path, fake_path=opt.fake_path, data_mode=opt.data_mode)
        ]
    ...
    for dataset_path in dataset_paths:
        ...
        ap, r_acc0, f_acc0, acc0, r_acc1, f_acc1, acc1, best_thres = validate(
            model, loader, find_thres=True, gpu_id=opt.gpu_id
        )
